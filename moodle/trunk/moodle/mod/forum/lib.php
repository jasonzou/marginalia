...
function forum_get_post_full($postid) {
    ...
    return get_record_sql("SELECT p.*, d.forum, u.firstname, u.lastname, u.username, u.email, u.picture
                            FROM {$CFG->prefix}forum_posts p
...
function forum_get_discussion_posts($discussion, $sort, $forumid) {
    ...
    return get_records_sql("SELECT p.*, $forumid AS forum, u.firstname, u.lastname, u.username, u.email, u.picture
                              FROM {$CFG->prefix}forum_posts p
                         LEFT JOIN {$CFG->prefix}user u ON p.userid = u.id
...
//	{
		...
		echo "<ol></ol>\n";
		$summaryUrl = $CFG->wwwroot."/annotation/summary.php?user=".urlencode($USER->username)."&url=".urlencode( $discussUrl );
//		if ( ANNOTATION_LOGUSER_URLS )
//			$summaryUrl .= "&loguser=".urlencode($USER->username);
//		echo "<a class='annotation-summary' href='".htmlspecialchars($summaryUrl)."'>";
//		echo "annotation summary</a>\n";
		echo "</div></td>\n";
//	}
//	else
//		echo "<td></td><td></td>";
